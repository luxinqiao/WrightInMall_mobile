...
<template>
    <div id='main' ref='main' >
        <div id='head' :style="{display:(((browserType != 'wechat') && (!isTitle))?'block':'none')}">
            <div class='head_one_box'>
                <img :src="require('@/assets/images/common/ic_back2.png')" @click="back()" v-show='isGoback'>
                <img :src="require('@/assets/images/common/ic_share2.png')" @click="share()" v-show='isHasShare'>
            </div>
        </div>
        <div class='head_two'  id='head_two' :style="{display:(((browserType != 'wechat') && isTitle)?'block':'none')}">
            <div class='head_two_box'>
                <img :src='require("@/assets/images/common/backbtn.png")' class='back_btn' @click='back()' v-if='isGoback'>
                <div class='head_box'>
                ...
                <img :src='require("@/assets/images/common/home_share.png")' class='share_btn' @click='share()' v-if='isHasShare'>
            </div>
        </div>
        ...
        <div v-if='detailObj.is_release == "1"' ref='content'>
            ...
            <div class='top'>
                ...
                <!-- 活动 -->
                <div class='activity' v-if='detailObj.mark.status==1&&paramObj.goods_type==1'>{{detailObj.mark.content}}</div>

                <!-- 价格 -->
                <div class='price_sale'>
                    <div class='price wm_margin' v-if='paramObj.goods_type==1||paramObj.goods_type==4'>
                        <span class='wm_color_main'>￥</span><span class='wm_color_main'>{{price}}</span><span v-if='!isEmpty(originPrice)&&originPrice!=0&&originPrice!=price'>￥{{originPrice}}</span>
                    </div>
                    <div class='price wm_margin' v-if='paramObj.goods_type==3'>
                ...
                <!-- 优惠券 -->
                <div @click='openPop("coupon")' v-if='!isEmpty(couponArr)&&detailObj.is_release==1&&paramObj.goods_type==1' class='list list_nomal coupon'>
                    <div class='list_tit'>优惠</div>
                    <div class='cou_box'>
                        <div class='cou_item wm_color_main' v-if='couponArr.length>=1'>{{priceTransition(couponArr[0].amount)}}元优惠券</div>
                        <div class='cou_item wm_color_main' v-if='couponArr.length>=2'>{{priceTransition(couponArr[1].amount)}}元优惠券</div>
                    </div>
                    <div class='get_cou'>
                        <span>领券</span>
                        <img v-lazy='require("@/assets/images/mall/details/next.png")'>
                    </div>
                </div>
                <!-- 活动 -->
            ...
            <div class='middle'>
                <div class='address list list_nomal'>
                    ...
                    <div>{{detailObj.express_price>0?priceTransitionTwo(detailObj.express_price):'包邮'}}</div>
        ...
        <mdPopup position='bottom' :isShow='isImgPre' :isRound='false' @overlayClose='closeImgPre'>
            <div class='download_img'>
                <div @click.stop='downloadImg'>保存图片</div>
        ...
        <mdPopup :isShow='isCoupon' @overlayClose='closePop("coupon")'>
            <couponDialog @closeCoupon='closePop("coupon")'></couponDialog>
        </mdPopup>
            ...
            <div class='pop_box style_pop'>
                ...
                <div class='style_con' scroll-y >
                    <div class='info'>
                        ...
                        <div>
                            <div class='wm_color_main' v-if='paramObj.goods_type==1||paramObj.goods_type==4'>￥{{!isEmpty(styleCode)?priceTransitionTwo(stylePrice):price}}</div>
                            <div class='wm_color_main' v-if='paramObj.goods_type==3'>￥{{!isEmpty(styleCode)?priceTransitionTwo((stylePrice-reducePrice)>0?(stylePrice-reducePrice):0):rePrice}}</div>
                            <div>{{styleChoose}}</div>
                        </div>
                    </div>
                    <div class='style'>
...
<script type='text/javascript'>
    ...
    import {priceTransition, priceTransitionTwo, getParameter, getBrowserType, isEmpty, getLocalToken, convert23, setWechatRole, urlChange, appFun, getAppVersion, appShare, downloadIamge, getAppHeader} from '@/utils/CommonUtils.js'
    import couponDialog from '@/page/main/mall/dialog/couponDialog'
    ...
    export default {
        ...
        mounted() {
            ...
            this.paramObj.goods_type = isEmpty(this.paramObj.goods_type)?'1':this.paramObj.goods_type

            if (this.browserType == 'browser') {
                this.isHasShare = false
                if (this.paramObj.type=='LT01') {
                    this.isGoback = false
                }
            ...
            if (getBrowserType() == 'wechat' && isEmpty(this.$route.query.openid) && getParameter('type')=='LT01') { //跳转微信授权
                let url = convert23(host_path)+'mall/details?goods_type='+this.paramObj.goods_type+'&goods_code='+getParameter('goods_code')+'&type=LT01'
                if(!isEmpty(getParameter('referral_code'))) {
                    url = url+ '&referral_code='+getParameter('referral_code')
                }
                if(!isEmpty(getParameter('sn'))) {
                    url = url+ '&sn='+getParameter('sn')
                }
            ...
            this.initAppTop('head')
            this.initAppTop('head_two')
        },
        methods: {
            /**
                app内顶部距离
            ...
            */
            initAppTop(id) {
                if(this.browserType =='app' && !isEmpty(getAppHeader()) && document.getElementById(id)) {
                    document.getElementById(id).style = `padding-top:${getAppHeader()}px`
                }
            },
            /**
                监听页面滚动
                ...
                .then(response => {
                    if (response.code == 200) {
                        ...
                        if(this.detailObj.is_release == '1') {
                            ...
                            if(this.paramObj.goods_type=='1' || this.paramObj.goods_type=='4'){
                                this.initPrice()
                            }
                            ...
                            sessionStorage.wm_title = this.detailObj.name
                            sessionStorage.wm_desc = '这个商品还不错，推荐给您！'
                            sessionStorage.wm_icon = this.detailObj.mainimgs[0]
                            sessionStorage.wm_link = window.location.protocol+ '//' + window.location.host + '/#/mall/details?goods_code='+ this.paramObj.goods_code +'&goods_type=1&type=LT01';
                            sessionStorage.wm_type = ''

                            if(this.version) {
                                //app分享-新版分享-新增
                                sessionStorage.wm_share_type = '502'
                                sessionStorage.wm_share_code = this.paramObj.goods_code
                            }
            ...
            */
            initPrice() {
                ...
                if(minOriginal==maxOriginal){
                    this.originPrice = priceTransitionTwo(minOriginal)
                }else if(minOriginal!=maxOriginal){
                    this.originPrice = priceTransitionTwo(minOriginal)+'-'+priceTransitionTwo(maxOriginal)
                }
            },
            /**
                初始化推荐商品
            ...
            */
            buyCheck() {
                ...
                let path = `../mall/confirm?goods_code=${this.paramObj.goods_code}&goods_type=${this.paramObj.goods_type}&styleCode=${this.styleCode}&num=${this.num}`
                if(!isEmpty(this.severCode)) {
            ...
            */
            initCoupon() {
                if(isEmpty(getLocalToken())){
                    ...
                    this.$axios.post(live_request+'v1/coupon/canusedetail', {
                        token: getLocalToken(),
                        for_where: 12,
                        where_code: getParameter('goods_code'),
                    })
            ...
            */
            back() {
                ...
                //商城根目录：关闭页面
                if ((path=='/mall/details')&&(this.$route.query.is_root=='1')) {
                    closeWindow()
                    if(this.browserType != 'app') {
                        history.back(-1)
                    }
                } else if ((path=='/mall/details')&&(this.$route.query.type=='LT01')) {
                    closeWindow()
                } else {
                    history.back(-1)
                }
    ...
    #main {
        ...
        .coupon {
            ...
            .cou_box {
                ...
                .cou_item {
                    position: relative;
                    margin-right: 0.08rem; padding: 0 0.11rem;
                    min-width: 0.74rem; height: 0.2rem;
                    text-align: center; line-height: 0.2rem;
                    font-size: 0.11rem;
                    background: url(../../../assets/images/mall/details/coupon.png) no-repeat;
            ...
            .get_cou {
                ...
                background: linear-gradient(90deg, #FF2C79 0%, #FFA082 100%);
        ...
        .detail_quest {
            ...
            .detail_title {
                ...
                font-size: 0.15rem; color: #333; font-weight: bold;
